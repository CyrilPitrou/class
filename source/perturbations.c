    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) { /* if radiation streaming approximation is off */
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) { /* if tight-coupling approximation is off */
        ...
        case tam:
          ...
          class_stop(ppr->error_message,"For vector modes, the TAM hierarchy of photons is not written yet");
          break;
        }
      }
    }
  ...
  if (_tensors_) {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) { /* if radiation streaming approximation is off */
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
        ...
        case tam:
        /* tam: we only need Theta_2, Theta_4, E2 */
          ppv->used_in_sources[ppv->index_pt_l2_g+1]=_FALSE_;
    ...
    if (_tensors_) {
      ...
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        case tam:
          ...
          // C. Pitrou (2020): use tight-coupling relation Theta_2^(2) = -4/3 * H'/kappa'
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) { /* if radiation streaming approximation is off */
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) { /* if tight-coupling approximation is off */
        ...
        case optimal:
          ...
          // todo: re-check the overall sign
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
      ...
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
        ...
        /** - -----> photon polarization closure, using (2.34) of 1305.3261 (otpimal)
            or Pitrou-Riazuelo closure relation (9,10) of 2005.xxxxx (tam).
            We also keep the -\kappa'*multipole term, but this is actually irrelevant since
            the closure is precisely meant to be used when there is free streaming. */
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ur == _TRUE_) {
      ...
      case tam:
        ...
        /* derivative of Theta_2^(2) using (33) of astro-ph/9709066 with l=2 m=2 */
        dy[pv->index_pt_l2_ur] = sqrt(MAX(0,k2+3.*pba->K))*
          (-zerokappam[3]/7.*y[pv->index_pt_l2_ur+1])
          -y[pv->index_pt_gwdot];

        /* derivative of Theta_l^(2) using (33) of astro-ph/9709066 with l>2 m=2 */
        for (l=3; l < pv->l_max_ur; l++)
          dy[pv->index_pt_l2_ur+l-2] = sqrt(MAX(0,k2+3.*pba->K))*
            (-zerokappam[l+1]/(2.*l+3)*y[pv->index_pt_l2_ur+l-1]
             +zerokappam[l]/(2.*l-1)*y[pv->index_pt_l2_ur+l-3]);

        /* Pitrou-Riazuelo closure relation using (9) of 2005.xxxxx */
        l = pv->l_max_ur;
        dy[pv->index_pt_l2_ur+l-2] = (sqrt(MAX(0,k2+3.*pba->K))*
                                        (zerokappam[l]*(2.*l+1.)/(2.*l-1.)/(l-2.)*y[pv->index_pt_l2_ur+l-3]))
          -(l+3.)*k*cotKgen*y[pv->index_pt_l2_ur+l-2];
        break;
      }
    }
